...
CREATE OR REPLACE PROCEDURE GOLD.SP_LOAD_DIM_CUSTOMER()
    ...
    AS 
    BEGIN 
        LET ETL_START_DATE DATE := (SELECT ETL_START_DATE FROM METADATADB.PUBLIC.ETL_CONFIG_TABLE WHERE STATUS = 1);
        LET ETL_END_DATE DATE := (SELECT ETL_END_DATE FROM METADATADB.PUBLIC.ETL_CONFIG_TABLE WHERE STATUS = 1);
        ...
        SET
            ...
            TGT.DWH_END_DATE = DATEADD("DAY",-1,''' || ETL_START_DATE || ''') -- VARIABLE TO LOAD DATE
...
FROM 
    SALES.SILVER.POS_SALES_DETAILS SLS
